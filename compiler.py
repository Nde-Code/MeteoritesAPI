...
def is_invalid_location(
    lat: float | None, lon: float | None, geolocation: str | None
) -> bool:
...
def main():
    ...
    with input_path.open(newline="", encoding="utf-8") as csvfile:
        ...
        for row_index, row in enumerate(reader, start=1):
            ...
            if any(field not in row or row[field] is None for field in EXPECTED_FIELDS):
                print(
                    f"ERROR: Invalid row structure at line {row_index + 1}. "
            ...
            if args.clean_up and is_invalid_location(lat_val, lon_val, geo_location):
                debug_print(
                    2,
                    args.debug,
                    f"Clean-up: removed meteorite ({row.get('name', 'Unknown')}) with invalid or missing location",
                )
                continue
    ...
    data_dict = {
        "meteorites": {f"meteorite_{i + 1}": data for i, data in enumerate(data_list)}
    }
