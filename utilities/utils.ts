...
export function isConfigValidWithMinValues(
    config: StaticConfig,
    rules: Partial<Record<keyof StaticConfig, number>>
): boolean {
...
export async function applyRateLimit(req: Request, currentConfig: RuntimeConfig): Promise<Response | null> {
    ...
    if (!(await checkTimeRateLimit(hashedIP, currentConfig.RATE_LIMIT_INTERVAL_S)))
        return createJsonResponse(
            { warning: `Rate limit exceeded: only 1 request per ${currentConfig.RATE_LIMIT_INTERVAL_S}s allowed.` },
            429
        );
