...
import type { HealthCheckResult } from '../types/types.ts';
...
export async function handleHealthCheck(): Promise<Response> {
    ...
    const allHealthy: boolean =
        checks.cache_ready &&
        checks.meteorites_count > 0 &&
        checks.shuffled_meteorites_count > 0 &&
        checks.stats_available &&
        checks.by_id_index_size > 0 &&
        checks.by_name_index_size > 0;
