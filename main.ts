...
import type { Env, StaticConfig, RuntimeConfig, Filters } from './types/types.ts';

import type { Meteorite, Meteorites } from './types/meteorites.ts';
...
async function handler(req: Request, env: Env): Promise<Response> {
    ...
    const isConfigValid =
        isConfigValidWithMinValues(currentConfig, configMinValues) &&
        currentConfig.DEFAULT_RANDOM_NUMBER_OF_METEORITES <= currentConfig.MAX_RANDOM_METEORITES;
    ...
    if (!isConfigValid)
        return createJsonResponse({ error: 'Invalid configuration detected. Please refer to the documentation.' }, 500);
    ...
    if (req.method === 'GET' && pathname === '/')
        return createJsonResponse(
            {
                success:
                    'Welcome to the API root. See the project repository https://github.com/Nde-Code/MeteoritesAPI for further information.'
            },
            200
        );
    ...
    if (req.method === 'GET' && pathname === '/random') {
        ...
        let requestedCount: number = countParam
            ? parseInt(countParam)
            : currentConfig.DEFAULT_RANDOM_NUMBER_OF_METEORITES;
        ...
        if (actualLimit > currentConfig.MAX_RANDOM_METEORITES || actualLimit <= 0)
            return createJsonResponse(
                { error: `The number of meteorites must be between 1 and ${currentConfig.MAX_RANDOM_METEORITES}.` },
                400
            );
        ...
        printLogLine(
            'INFO',
            `Returned ${randomMeteorites.length} meteorites from ${pathname + url.search} (start index: ${start}).`
        );
    ...
    if (req.method === 'GET' && pathname === '/get') {
        ...
        if (id && !/^[1-9]\d*$/.test(id))
            return createJsonResponse({ error: 'The ID must be a positive integer.' }, 400);
    ...
    if (req.method === 'GET' && pathname === '/search') {
        ...
        if (
            ...
            filters.mass === null &&
            filters.minMass === null &&
            filters.maxMass === null &&
            filters.centerLat === null &&
            filters.centerLon === null &&
            filters.radius === null
        )
            return createJsonResponse(
                { error: 'At least one valid filter parameter is required in the search query.' },
                400
            );

        if (filters.fall && filters.fall.toLowerCase() !== 'fell' && filters.fall.toLowerCase() !== 'found')
            return createJsonResponse({ error: "The fall filter must be set to either 'fell' or 'found'." }, 400);

        if (filters.year !== null && (filters.minYear !== null || filters.maxYear !== null))
            return createJsonResponse({ error: "Cannot combine 'year' with 'minYear' or 'maxYear'." }, 400);

        if (filters.mass !== null && (filters.minMass !== null || filters.maxMass !== null))
            return createJsonResponse({ error: "Cannot combine 'mass' with 'minMass' or 'maxMass'." }, 400);

        if (
            filters.radius !== null &&
            (filters.radius < currentConfig.MIN_RADIUS || filters.radius > currentConfig.MAX_RADIUS)
        )
            return createJsonResponse(
                {
                    error: `The radius must be between ${currentConfig.MIN_RADIUS} km and ${currentConfig.MAX_RADIUS} km.`
                },
                400
            );

        if (
            (filters.centerLat !== null || filters.centerLon !== null || filters.radius !== null) &&
            !(filters.centerLat !== null && filters.centerLon !== null && filters.radius !== null)
        )
            return createJsonResponse(
                {
                    error: 'Incomplete geographic parameters: centerLatitude, centerLongitude, and radius must all be provided together.'
                },
                400
            );

        if (filters.limit !== null && !isPositiveInteger(filters.limit))
            return createJsonResponse({ error: 'The limit parameter must be a positive integer.' }, 400);

        const limit: number =
            filters.limit !== null && isPositiveInteger(filters.limit)
                ? Math.min(filters.limit, currentConfig.MAX_RETURNED_SEARCH_RESULTS)
                : currentConfig.MAX_RETURNED_SEARCH_RESULTS;
        ...
        if (useLocation) {
            if (filters.centerLat! < -90 || filters.centerLat! > 90)
                return createJsonResponse({ error: 'Latitude must be between -90 and 90.' }, 400);

            if (filters.centerLon! < -180 || filters.centerLon! > 180)
                return createJsonResponse({ error: 'Longitude must be between -180 and 180.' }, 400);
        ...
        printLogLine(
            'INFO',
            `Returned ${results.length} meteorite${results.length !== 1 ? 's' : ''} from ${pathname + url.search}.`
        );
